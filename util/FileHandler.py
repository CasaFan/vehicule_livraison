...
class FileHandler:
    ...
    def open_init_file(self):
        """
        Ouvrir et parser le fichier .ini
        :return: un objet de VehicleConfiguration
        """
        ...
        try:
            ...
            vehicle_configuration = VehicleConfiguration(
                ...
                int(self.get_time_in_seconde(config['Vehicle']['start_time'])),
                int(self.get_time_in_seconde(config['Vehicle']['end_time']))
            )
            return vehicle_configuration

    def check_ariables(self, config):
        """
        Un validateur de fichier .ini
        :param config: une liste de 2 dimensions parsé par ConfigParser
        :return:
        """
    ...
    def open_data_directory(self):
        """
        Demande d'entrée d'un répertoire qui contient les fichiers data et les charger.
        :return: a dataset
        """
    ...
    def get_time_in_seconde(self, time_str):
        """
        parse un time string to seconds
        :param time_str: string
        :return: total seconds of a time string
        """
